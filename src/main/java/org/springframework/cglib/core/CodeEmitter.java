...
public class CodeEmitter extends LocalVariablesSorter {
	private static final Signature BOOLEAN_VALUE = TypeUtils.parseSignature((String) "boolean booleanValue()");
	private static final Signature CHAR_VALUE = TypeUtils.parseSignature((String) "char charValue()");
	private static final Signature LONG_VALUE = TypeUtils.parseSignature((String) "long longValue()");
	private static final Signature DOUBLE_VALUE = TypeUtils.parseSignature((String) "double doubleValue()");
	private static final Signature FLOAT_VALUE = TypeUtils.parseSignature((String) "float floatValue()");
	private static final Signature INT_VALUE = TypeUtils.parseSignature((String) "int intValue()");
	private static final Signature CSTRUCT_NULL = TypeUtils.parseConstructor((String) "");
	private static final Signature CSTRUCT_STRING = TypeUtils.parseConstructor((String) "String");
	...
	public CodeEmitter(CodeEmitter wrap) {
		super((LocalVariablesSorter) wrap);
		this.ce = wrap.ce;
	...
	public void push(int i) {
		if (i < -1) {
			...
			this.mv.visitInsn(TypeUtils.ICONST((int) i));
		} else if (i <= 127) {
	...
	public void push(long value) {
		if (value == 0L || value == 1L) {
			this.mv.visitInsn(TypeUtils.LCONST((long) value));
		} else {
	...
	public void push(float value) {
		if (value == 0.0f || value == 1.0f || value == 2.0f) {
			this.mv.visitInsn(TypeUtils.FCONST((float) value));
		} else {
	...
	public void push(double value) {
		if (value == 0.0 || value == 1.0) {
			this.mv.visitInsn(TypeUtils.DCONST((double) value));
		} else {
	...
	public void newarray(Type type) {
		if (TypeUtils.isPrimitive((Type) type)) {
			this.mv.visitIntInsn(188, TypeUtils.NEWARRAY((Type) type));
		} else {
	...
	public void load_this() {
		if (TypeUtils.isStatic((int) this.state.access)) {
	...
	public void getfield(String name) {
		...
		int opcode = TypeUtils.isStatic((int) info.access) ? 178 : 180;
		this.emit_field(opcode, this.ce.getClassType(), name, info.type);
	}

	public void putfield(String name) {
		ClassEmitter.FieldInfo info = this.ce.getFieldInfo(name);
		int opcode = TypeUtils.isStatic((int) info.access) ? 179 : 181;
		this.emit_field(opcode, this.ce.getClassType(), name, info.type);
	}
	...
	private void emit_type(int opcode, Type type) {
		String desc = TypeUtils.isArray((Type) type) ? type.getDescriptor() : type.getInternalName();
	...
	public void box(Type type) {
		if (TypeUtils.isPrimitive((Type) type)) {
			if (type == Type.VOID_TYPE) {
				...
				Type boxed = TypeUtils.getBoxedType((Type) type);
	...
	public void visitMaxs(int maxStack, int maxLocals) {
		if (!TypeUtils.isAbstract((int) this.state.access)) {
	...
	public void invoke(MethodInfo method, Type virtualType) {
		...
		} else if (TypeUtils.isInterface((int) classInfo.getModifiers())) {
			this.invoke_interface(type, sig);
		} else if (TypeUtils.isStatic((int) method.getModifiers())) {
	...
	private static class State extends MethodInfo {
		...
		State(ClassInfo classInfo, int access, Signature sig, Type[] exceptionTypes) {
			...
			this.localOffset = TypeUtils.isStatic((int) access) ? 0 : 1;
